<!DOCTYPE html>

<html lang="pt-br">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />

  <link rel="stylesheet" href="style.css?v=<v?=time() ?>" />
  <title>GovernoWeb</title>
  <link rel="stylesheet" href="./fontawesome/fontawesome-pro.css" />
  <link href="https://fonts.googleapis.com/css2?family=Montserrat:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;0,700;0,800;0,900;1,100;1,200;1,400;1,500;1,600;1,700;1,800;1,900&display=swap" rel="stylesheet">
</head>

<body>
  <!-- CABEÇALHO -->
  <section id="hero">
    <header>
      <img src="./assets/logo-governo.png" alt="logo governo web" />
      <nav class="menu">
        <ul class="menu-ul">
          <li><a href="#hero">Início</a></li>
          <li><a href="#solucoes">Benefícios</a></li>
          <li><a href="#clientes">Clientes</a></li>
          <li><a href="#solucao">Soluções</a></li>
          <li><a href="#Depoimentos">Depoimentos</a></li>

          <li>
            <button class="whatsapp">
              <a href="https://api.whatsapp.com/send?phone=5531984649775&text=Ol%C3%A1,%20gostaria%20de%20solicitar%20um%20or%C3%A7amento!" target="_blank">
                <i class="far fa-user" style="margin-right: 5px"></i> Falar
                com Atendente
              </a>
            </button>
          </li>
        </ul>
      </nav>
    </header>
    <div class="conteudo">
      <div class="texto">
        <h1>
          Conheça as melhores e mais completas soluções para o Poder
          Legislativo
        </h1>
        <p>
          A GovernoWeb é uma plataforma online para interação de pessoas e
          setores, especialmente criada para otimizar as rotinas de trabalho
          em câmaras municipais de todos os portes.
        </p>

        <button>
          <a href="https://api.whatsapp.com/send?phone=5531984649775&text=Ol%C3%A1,%20gostaria%20de%20solicitar%20um%20or%C3%A7amento!" target="_blank">Saiba mais<i class="fas fa-angle-down" style="margin-left: 10px"></i></a>
        </button>
      </div>
      <div class="img">
        <img src="./assets/img-tela.png" alt="foto da tela de login" />
      </div>
    </div>
  </section>


  <section id="solucoes" class="beneficios">
    <div class="conteudo">
      <h1>Alguns benefícios</h1>
      <div class="container-beneficios">
        <div class="container-items">
          <i class="fal fa-file-invoice"></i>Geração de doocumentos
          padronizados
        </div>
        <div class="container-items">
          <i class="fas fa-hand-point-up"></i>Fácil utilização, com layout
          intuitivo
        </div>
        <div class="container-items">
          <i class="fal fa-shield-check"></i>Segurança das informações
        </div>
        <div class="container-items">
          <i class="far fa-print-search"></i>Mais transparência na produção
          legislativa
        </div>

        <div class="container-items">
          <i class="fal fa-text-width"></i>Editor de texto integrado
        </div>
        <div class="container-items">
          <i class="far fa-paste"></i>Protocolo externo e em lote
        </div>
        <div class="container-items">
          <i class="fas fa-cloud-upload"></i>Gestão documental com backups de
          segurança
        </div>
        <div class="container-items">
          <i class="far fa-file-certificate"></i>Validade juridica com
          certificação ICP Brasil
        </div>
      </div>
    </div>
  </section>

  <section id="clientes" class="clientes">

    <h2>Conheça alguns de nossos clientes</h2>
    <div class="slider">
      <div class="slides">
        <input type="radio" name="radio-btn" id="radio1" />
        <input type="radio" name="radio-btn" id="radio2" />
        <input type="radio" name="radio-btn" id="radio3" />
        <input type="radio" name="radio-btn" id="radio4" />

        <div class="slide first">
          <img src="assets/aguaboa.png" alt="imagem 1" />
          <img src="assets/aimores.png" alt="imagem 1" />
          <img src="assets/alvoradademinas.png" alt="imagem 1" />
        </div>

        <div class="slide">
          <img src="assets/centraldeminas.png" alt="imagem 1" />
          <img src="assets/conselheiropena.png" alt="imagem 1" />
          <img src="assets/freiinocencio.png" alt="imagem 1" />
        </div>

        <div class="slide">
          <img src="assets/galileia.png" alt="imagem 1" />
          <img src="assets/novabelem.png" alt="imagem 1" />
          <img src="assets/itaobim.png" alt="imagem 1" />
        </div>

        <div class="slide">
          <img src="assets/manga.png" alt="imagem 1" />
          <img src="assets/miravania.png" alt="imagem 1" />
          <img src="assets/nanuque.png" alt="imagem 1" />
        </div>

        <div class="navigation-auto">
          <div class="auto-btn1"></div>
          <div class="auto-btn2"></div>
          <div class="auto-btn3"></div>
          <div class="auto-btn4"></div>
        </div>
      </div>

      <div class="manual-navigation">
        <label for="radio1" class="manual-btn"></label>
        <label for="radio2" class="manual-btn"></label>
        <label for="radio3" class="manual-btn"></label>
        <label for="radio4" class="manual-btn"></label>
      </div>
    </div>
  </section>

  <section id="solucao" class="sistemas">
    <div class="conteudo">
      <div class="container-sistemas">
        <h1>
          <i class="fal fa-clouds" style="margin-right: 8px;"></i>Soluções
        </h1>
        <div class="items">
          <div class="wrap">
            <p>
              <i class="fas fa-arrow-right" style="margin-right: 8px;"></i>Maior agilidade no processo de presença
              votação
            </p>
            <p>
              <i class="fas fa-arrow-right" style="margin-right: 8px;"></i>Total trasnparência nas atividades
              legislativas
            </p>
            <p>
              <i class="fas fa-arrow-right" style="margin-right: 8px;"></i>Otimização das atividades parlamentares
            </p>
            <p>
              <i class="fas fa-arrow-right" style="margin-right: 8px;"></i>Maior visibilidade dos trabalhos em lenário
            </p>
            <p>
              <i class="fas fa-arrow-right" style="margin-right: 8px;"></i>Emissão de documentos e relatórios em tempo
              real
            </p>
            <p>
              <i class="fas fa-arrow-right" style="margin-right: 8px;"></i>Viabilização do plenário para atividades
              extras
              parlamentares
            </p>
            <p>
              <i class="fas fa-arrow-right" style="margin-right: 8px;"></i>Modernização da casa legislativa
            </p>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section id="Depoimentos" class="clientes">

    <h2>Depoimentos</h2>
    <div class="slider">
      <div class="slides">
        <input type="radio" name="radio-btn" id="radio1" />
        <input type="radio" name="radio-btn" id="radio2" />
        <input type="radio" name="radio-btn" id="radio3" />
        <input type="radio" name="radio-btn" id="radio4" />

        <div class="slide first">
          <img src="assets/aguaboa.png" alt="imagem 1" />
          <img src="assets/aimores.png" alt="imagem 1" />
          <img src="assets/alvoradademinas.png" alt="imagem 1" />
        </div>

        <div class="slide">
          <img src="assets/centraldeminas.png" alt="imagem 1" />
          <img src="assets/conselheiropena.png" alt="imagem 1" />
          <img src="assets/freiinocencio.png" alt="imagem 1" />
        </div>

        <div class="slide">
          <img src="assets/galileia.png" alt="imagem 1" />
          <img src="assets/novabelem.png" alt="imagem 1" />
          <img src="assets/itaobim.png" alt="imagem 1" />
        </div>

        <div class="slide">
          <img src="assets/manga.png" alt="imagem 1" />
          <img src="assets/miravania.png" alt="imagem 1" />
          <img src="assets/nanuque.png" alt="imagem 1" />
        </div>

        <div class="navigation-auto">
          <div class="auto-btn1"></div>
          <div class="auto-btn2"></div>
          <div class="auto-btn3"></div>
          <div class="auto-btn4"></div>
        </div>
      </div>

      <div class="manual-navigation">
        <label for="radio1" class="manual-btn"></label>
        <label for="radio2" class="manual-btn"></label>
        <label for="radio3" class="manual-btn"></label>
        <label for="radio4" class="manual-btn"></label>
      </div>
    </div>
  </section>

  <footer>
    <div class="container-footer">
      <div class="rede-logo">
        <img src="assets/logo-governo.png" />
        <div class="rede-social">
          <p>Acompanhe-nos</p>
          <a href="https://www.instagram.com/governo_web/" target="_blank"><i class="fab fa-instagram"></i></a>
        </div>
      </div>
      <div class="atendimento">
        <p>Atendimento</p>
        <div class="container-atendimento">
          <div class="horario">
            <span><a href="tel:+5531984649775"><i class="fas fa-phone" style="margin-right: 8px;"></i>(31)
                98464-9775</a></span>
            <span><i class="fas fa-clock" style="margin-right: 8px;"></i>Seg a sex - 08:00 / 17:30</span>
          </div>
          <div class="endereco">
            <span><a href="mailto:email@provedor.com.br"><i class="fas fa-envelope" style="margin-right: 8px;"></i>contato@governoweb.com.br</a></span>
            <span><i class="fas fa-map-marker-alt" style="margin-right: 8px;"></i>Rua Peçanha, 848 - Sl 06 - Centro
              <br>Governador Valadares - MG</span>
          </div>
        </div>
      </div>
    </div>
    <div class="direitos">© 2023 GWLEGIS - TODOS OS DIREITOS RESERVADOS</div>
  </footer>

  <a href="#hero" class="botao-subir"><i class="fas fa-caret-up"></i></a>

  <a href="https://api.whatsapp.com/send?phone=5531984655356&text=Ol%C3%A1,%20Gostaria%20de%20fazer%20um%20agendamento!" class="btn-whatsapp" target="_blank" data-aos="zoom-in-up" data-aos-delay="400">
    <img src="assets/whatssapp.svg" alt="Botao whatssapp" />

  </a>

  <script src="script.js"></script>
</body>

</html>